...
import { hasActiveSubscription, isSubscriptionsEnabled } from "@/lib/subscription";
...
export async function POST(
  ...
  try {
    ...
    if (hasAuthCredentials(request)) {
      ...
      // On-demand generation requires subscription when billing is enabled
      if (
        ...
        return NextResponse.json(
          { error: "On-demand AI report generation requires an active subscription." },
          { status: 403 }
        );
      }
