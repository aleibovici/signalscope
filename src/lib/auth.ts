...
import { isSubscriptionsEnabled, API_KEY_DAILY_LIMIT } from "@/lib/subscription";
...
export async function getCurrentUserId(): Promise<string> {
  ...
  if (apiKey) {
    ...
    if (record && !record.revokedAt) {
      ...
      const hasFullAccess =
        !isSubscriptionsEnabled() || subStatus === "ACTIVE" || subStatus === "PAST_DUE";

      if (hasFullAccess) {
        if (isApiKeyRateLimited(record.userId)) {
          throw new Error(
            `API key rate limit exceeded (${API_KEY_DAILY_LIMIT.toLocaleString()} requests/day)`
          );
        }
      } else {
