...
export function buildWeeklyDigestHtml(
  ...
  function renderPerformer(p: DigestPerformer): string {
    ...
    return `<span style="display:inline-block;margin-right:16px;"><a href="https://signalscopes.com/ticker/${p.symbol}" style="color:#2563eb;font-weight:600;text-decoration:none;">$${p.symbol}</a> <span style="color:${color};font-weight:600;">${formatPct(p.returnPct)}</span> <span style="color:#6b7280;font-size:12px;">(${p.period})</span></span>`;
  }
