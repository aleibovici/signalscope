...
export function buildEmailHtml(tickers: AlertTicker[], totalAvailable?: number): string {
  ...
  function renderSection(label: string, items: AlertTicker[], color: string, maxItems?: number): string {
    ...
    const moreLink = remaining > 0
      ? `<tr><td colspan="3" style="padding:8px 12px;border-bottom:1px solid #e5e7eb;"><a href="http://localhost:3000/dashboard" style="color:#2563eb;font-size:13px;text-decoration:none;">+${remaining} more emerging signal${remaining !== 1 ? "s" : ""} — view on dashboard →</a></td></tr>`
      : "";
  ...
  <div style="max-width:600px;margin:0 auto;padding:24px;">
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;padding:16px 24px;">
      ...
      <p style="margin:16px 0 8px;font-size:13px;color:#6b7280;">
        <a href="http://localhost:3000/portfolio" style="color:#2563eb;text-decoration:none;">View your portfolio →</a>
      </p>
      <p style="margin:0;font-size:12px;color:#9ca3af;">
        You're receiving this because email alerts are enabled on your SignalScope profile.
        To unsubscribe, disable alerts in your <a href="http://localhost:3000/profile" style="color:#9ca3af;">profile settings</a>.
