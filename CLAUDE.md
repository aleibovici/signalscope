...
```bash
# Manual trigger:
curl -X POST http://localhost:3000/api/snapshots/collect -H "x-snapshot-key: <SNAPSHOT_API_KEY>"
...
gcloud scheduler jobs create http signalscope-reports \
  ...
  --schedule="15 9 * * 1-5" \
  --time-zone="America/New_York" \
  --uri="http://localhost:3000/api/reports/generate" \
  --http-method=POST \
  --headers="x-snapshot-key=<SNAPSHOT_API_KEY_VALUE>" \
...
gcloud scheduler jobs create http signalscope-performance-tweet \
  ...
  --schedule="0 10 * * 1-5" \
  --time-zone="America/New_York" \
  --uri="http://localhost:3000/api/tweets/performance" \
  --http-method=POST \
  --headers="x-snapshot-key=<SNAPSHOT_API_KEY_VALUE>" \
  --attempt-deadline=120s \
...
gcloud scheduler jobs create http signalscope-weekly-digest \
  ...
  --schedule="0 10 * * 0" \
  --time-zone="America/New_York" \
  --uri="http://localhost:3000/api/alerts/weekly-digest" \
  --http-method=POST \
  --headers="x-snapshot-key=<SNAPSHOT_API_KEY_VALUE>" \
  --attempt-deadline=300s \
...
Claude Agent Skill files are served from `public/skill/` (accessible at `http://localhost:3000/skill/`):
